...
#### **[LCV-D-Primary]**:
The light client has a local variable _primary_ that contains the Address (ID) of a full node.
...
correct headers (defined below) in the _State_ of the light client. 
...
- _trustThreshold_: a float. Can be used if correctness should not be based on more voting power and 1/3.
- _trustingPeriod_: a time duration [**[TMBC-TIME_PARAMS]**](TMBC-TIME_PARAMS-link).
- _clockDrift_: a time duration. Correction parameter dealing with only approximately synchronized clocks.
...
`VerifyHeaderAtHeight` is called, the signed header of the `trustedState` passed as input is in _State_. When the function returns a `TrustedState`, its signed header is added to _State_.
...
- Expected precondition
  - true
- Expected postcondition: 
  - If _n_ is correct and there is no error in the RPC to _n_: Returns the following data:
