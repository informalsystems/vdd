...
Given a height *h* as an input, the lite client lokaly stores the header of
...
**[TMBC]** The Tendermint blockchain is a list of headers with heights:
...
**[LCV-Seq-Live]** The verifier gets as input a height _h_, and eventually stores the header of height _h_ of the blockchain.
...
**[TFM-2THIRDS]** If a block _h_ is generated at time _bfttime_ (and this time is stored in the block), then a set of validators that hold more than 2/3 of the voting power in h.Header.NextV is correct until time h.Header.bfttime + tp.
...
**[LCV-A-TFM]** The Tendermint block chain satisfies the Tendermint failure model [TFM-2THIRDS].
...
**[LCV-VC-Inv]** Every header in _State_ was generated by an instance of Tendermint consensus.
...
_Remark_: The invariant [LCV-VC-Inv] and liveness requirement [LCV-VC-Life]
...
_Remark_: In liveness [LCV-VC-Life]we use "eventually", while in practice _trust(h)_ should be set to true before _h.Header.bfttime + tp_. If not, the block cannot be trusted because it is too old.
...
/\ "correct primary"  \
/\ _TrustedState.SignedHeader.Header.Time > now - tp_ \
/\ [LCV-A-Comm] /\ [TFM-CorrFull] /\ [LCV-VC-Life]  \
       -> [LCV-Seq-Live]
