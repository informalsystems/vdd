...
Given a height *h* as an input, the lite client stores the header of
...
**[LCV-Seq]** The verifier gets as input a height _h_, and eventually outputs the header of height _h_ of the blockchain.
...
**[LCV-A-Full]** The verifier communicates with a full node called _primary_. No assumption is made about the full node (it may be correct of faulty).
...
`VerifyHeaderAtHeight` checks timestamps, and in case the checks go through calls bisection: The function `VerifyBisection` implements
...
TODO: explain 1/3 rule here.

If `VerifySingle` is successful, it returns _TrustedState_ to `VerifyBisection` which returns _TrustedState_. Otherwise (and there is no fatal error), `VerifyBisection` computes a pivot height between _trustedState.height_ and _untrustedHeight, and goes into recursion.
